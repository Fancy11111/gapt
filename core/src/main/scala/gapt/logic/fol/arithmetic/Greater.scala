...
import gapt.expr.formula.constants.GreaterC
...
object Greater {
  def apply( a: Expr, b: Expr ): Atom = Apps( GreaterC( a.ty ), a, b ).asInstanceOf[Atom]
  ...
  def unapply( e: Expr ): Option[( Expr, Expr )] = e match {
    case expr.App( expr.App( GreaterC( _ ), a ), b ) => Some( a, b )
    case _ => None
  }
  ...
  def unapply( f: FOLExpression ): Option[( FOLTerm, FOLTerm )] = f.asInstanceOf[Expr] match {
    case GreaterEq( a: FOLTerm, b: FOLTerm ) => Some( a, b )
    case _                                   => None
  }
}
